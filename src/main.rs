...
const GRAVITY: f64 = 1.3;
const BGCOLOR: Color = Color::RGB(0,0,0);
const SCALE: usize = 1;
static mut BRUSH_RADIUS: usize = 20;
const BRUSH_CHANGE: usize = 5;
...
impl Particle {
    pub fn new(pos: Vec2::<usize>, material: FieldState) -> Self
    {
        Self {
            pos,
            updated: false,
            material,
            color: get_color_by_material(material),
            velocity: 1.0
        }
    }
}

fn get_color_by_material(material: FieldState) -> Color {
    match material {
        FieldState::Sand(_) => Color::RGB(194, 178, 128),
        FieldState::Water(_) => Color::RGB(84, 206, 246),
        FieldState::Wood(_) => Color::RGB(133, 94, 66),
        _ => BGCOLOR,
    }
}
...
fn get_corrected_pos(pos: usize, border: usize) -> usize {
    unsafe {
        match pos {
...
impl Sim {
    ...
    fn update_particle_pos(&mut self) {
        for id in (0..self.particles.len()).rev() {
            if !self.particles[id].updated {
                match self.particles[id].material {
                    FieldState::Sand(_) => {
                        self.update_movable_particle(id, &mut Sim::update_sand)
                    }
                    FieldState::Water(_) => {
                        self.update_movable_particle(id, &mut Sim::update_water)
                    }
                    _ => {}
                };
            }
    ...
    fn create_particles(&mut self, pos: Vec2<usize>) {
        match self.current_spawning_field {
            Some(FieldState::Wood(_)) => {
                self.create_particle_with_rules(pos, Sim::solid_particle_creation_rules);
            }
            Some(FieldState::Water(_)) | Some(FieldState::Sand(_)) => {
                self.create_particle_with_rules(
                    pos,
                    Sim::falling_particle_creation_rules,
                );
            },
            _ => {}
        }
    ...
    fn create_particle_with_rules(
        &mut self,
        pos: Vec2<usize>,
        creation_rules_fn: fn(&mut Sim, Vec2<usize>),
    ) {
...
impl App {
    pub fn new() -> Self {
        ...
        let window = get_sdl_window(&sdl_context, "sand", *WINDOW);
    ...
    pub fn input(&mut self) {
        for event in self.event_pump.poll_iter() {
            match event {
                ...
                => unsafe {
                    if BRUSH_RADIUS >= MAX_BRUSH {BRUSH_RADIUS = MAX_BRUSH}
                    else {
                        BRUSH_RADIUS += BRUSH_CHANGE
                    }
                },
                Event::KeyDown {keycode: Some(Keycode::Q), ..}
                => unsafe {
                    if BRUSH_RADIUS <= BRUSH_CHANGE {BRUSH_RADIUS = BRUSH_CHANGE}
                    else {
                        BRUSH_RADIUS -= BRUSH_CHANGE
                    }
                },
    ...
    pub fn render(&mut self) {
        clear_canvas_with_color(&mut self.canvas, BGCOLOR);
